...
export function TradingPairManager() {
  ...
  const checkTradingPairStatus = async () => {
    ...
    try {
      ...
      const isActive = await dexService.isTradingPairActive(formData.baseToken, formData.quoteToken)
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('gas') || error.message.includes('nonce')) {
          ...
          errorMessage = 'Insufficient funds for gas fees. Add more MONAD to your wallet.'
